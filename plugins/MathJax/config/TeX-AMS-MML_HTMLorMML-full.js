...
MathJax.Hub.Config({
                "HTML-CSS": {
                    availableFonts: ["Arial","Helvetica","sans-serif"],
                    preferredFont: "Arial", // force Web fonts                    
				...
				this.EmExSpan = e.Element("span", {
						style : {
							...
							"font-size-adjust" : "none"
									...
									className : "MathJax_mmlExBox"
								}
							], ["span", {
									className : "MathJax_MathML"
								}
							]]);
				f.math(f.mspace().With({
						width : "60ex"
					})).toNativeMML(this.EmExSpan.lastChild)
			}
...
(function (h, b, d) {
	...
	d.Augment({
		...
		preTranslate : function (p) {
			...
			q = false,
			w,
			n = this.config.linebreaks.automatic,
			u = this.config.linebreaks.width;
			if (n) {
...
MathJax.Hub.Register.StartupHook("HTML-CSS Jax Ready", function () {
	...
	a.mtable.Augment({
		toHTML : function (r) {
			...
			aP = [],
			aU;
			var G = b.FONTDATA.lineH * aH * I.useHeight,
			N = b.FONTDATA.lineD * aH * I.useHeight;
			...
			if (I.equalrows) {
				var aD = Math.max.apply(Math, ap),
				V = Math.max.apply(Math, at);
				for (ae = 0, ac = av.length; ae < ac; ae++) {
